...
class ELECRecDataset(Dataset):
    ...
    def _data_sample_rec_task(self, user_id, items, input_ids, target_pos, answer):
        ...
        multi_target_neg = []
        for _ in range(self.args.neg_numbers):
            target_neg = []
            seq_set = set(items)
            for cur_item in copied_input_ids:
                target_neg.append(neg_sample(cur_item, self.args.item_size))
            target_neg = [0] * pad_len + target_neg
            target_neg = target_neg[-self.max_len:]
            assert len(target_neg) == self.max_len
            multi_target_neg.append(torch.tensor(target_neg, dtype=torch.long))
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            ...
            return (cur_rec_tensors, seq_class_label)
        elif self.data_type == 'valid':
            cur_rec_tensors = self._data_sample_rec_task(user_id, items, input_ids, \
                                target_pos, answer)
            return cur_rec_tensors
...
if __name__ == '__main__':
    ...
    train_dataset = ELECRecDataset(args, user_seq, data_type='train')
